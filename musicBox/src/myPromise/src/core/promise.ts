// @ts-nocheck
import { Thenable, Promise } from "./index";
import {
  constructorError,
  resolverError,
  resolveSelfError,
...
export default class myPromise<R> implements Promise<R> {
    ...
    if (!Array.isArray(entries)) {
      ...
      return new this((resolve, reject) => {
        entries.forEach(item => {
          this.resolve(item).then((data: any) => {
            //注意这里的this.resolve和resolve是不一样的
            result.push(data);
            num++;
